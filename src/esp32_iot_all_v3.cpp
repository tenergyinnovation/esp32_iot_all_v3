 ...
 * RETURNED:    uint16_t
 ***********************************************************************/
uint16_t esp32_iot_all_v3::crc16_update(uint16_t crc, uint8_t a)
{
  int i;
...
/***********************************************************************
 * FUNCTION:    MIZTEK_Fan1p_ModbusRTU_begin
 * DESCRIPTION: set RX and TX pin
 * PARAMETERS:  rx, tx
 * RETURNED:    true/ false
 ***********************************************************************/
bool esp32_iot_all_v3::MIZTEK_Fan1p_ModbusRTU_begin(uint8_t rx, uint8_t tx)
{
  if (((tx == TXD2) || (tx == TXD3)) && ((rx == RXD2) || (rx == RXD3)))
  {
    rs485.begin(9600, SERIAL_8N1, rx, tx);
...
int8_t esp32_iot_all_v3::MIZTEK_Fan1p_ModbusRTU_searchAddress(void)
{
}
int8_t esp32_iot_all_v3::MIZTEK_Fan1p_ModbusRTU_setAddress(uint8_t id, uint8_t new_id)
{
}

/***********************************************************************
 * FUNCTION:    MIZTEK_Fan1p_ModbusRTU_CtlSpeed
 * DESCRIPTION: Control speed fan via Modbus RTU
 * PARAMETERS:  speed
 * RETURNED:    true/ false
 ***********************************************************************/
bool esp32_iot_all_v3::MIZTEK_Fan1p_ModbusRTU_CtlSpeed(uint8_t id, uint16_t speed)
{
  /*check parameter*/
  if (speed > 3200)
  {
    Serial.printf("Error: Fan speed is out of the range[0-3200 rpm]\r\n");
    return 0;
  }
  ...
  _data_write[2] = 0x68;
  _data_write[3] = 0x18;
  _data_write[4] = (speed & 0xFF00) >> 8;
  _data_write[5] = speed & 0x00FF;
...
/***** Debug monitor ****/
  ...
  if (_crc_r == _crc)
  {
    Serial.printf("Info: Success set fan speed => %d rpm\r\n", speed);
    return 1;
  }
  else
  {
